...
from three_d_scene_script.decoder_module import Commands, CommandTransformer, generate_square_subsequent_mask, select_parameters
...
def train_model(root_dir, base_save_dir, plot_save_dir, batch_size, num_epochs, experiment_name):
        ...
        for epoch in range(num_epochs):
            ...
            for point_cloud_paths, script_paths in dataloader:
                sparse_tensor, decoder_input_embeddings, gt_output_embeddings = prepare_data(
                    model, point_cloud_paths[0], script_paths[0], normalize=True
                )
                ...
                param_loss = criterion_parameters(output_parameters.view(-1, param_dim), gt_parameters.view(-1, param_dim))
                ...
                print(f"Batch {batch_count}, Loss: {total_loss_batch.item()}, Command Loss: {command_loss.item()}, Param Loss: {param_loss.item()}")
            ...
            print(f"Epoch {epoch + 1} completed. Avg Loss: {avg_epoch_loss[-1]}, Command Loss: {avg_command_loss[-1]}, Param Loss: {avg_parameter_loss[-1]}\n")
